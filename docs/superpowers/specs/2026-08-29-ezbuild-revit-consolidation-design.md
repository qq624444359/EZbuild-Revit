...
### D15 — Revit 2027 是 .NET 10，恢复双目标（2026-09-03，修正 D14 末尾的假设）

D14 结尾标注了一处未验证假设：「按 Revit 2026 与 2027 同为 .NET 8 处理」。
作者确认**Revit 2027 用 .NET 10**，该假设不成立，单目标方案随之作废。

`EZTable.csproj` 恢复双目标，但框架与上一版完全不同——不再是「新旧 .NET」，
而是**一个受支持的 Revit 对应一个运行时**：
...
* `RevitVersion` 不再是可覆盖的单值，而是**由目标框架推导**（net8.0→2026，
  net10.0→2027），`RevitApiDir` 随之各指各的 Revit 安装目录
* `PostBuild` 条件从「仅 net8.0」放宽为「任一目标」，两个目标各自把产物拷进
  自己那版 Revit 的 `Addins\<版本>\`
* `CheckRevitApi` 的报错信息带上当前目标框架，并提示用
  `-p:TargetFrameworks=` 只编手头有的那个
* 两份 README 恢复目标框架表（三列，含运行时），构建示例改为按 Revit 版本
  二选一；**并写明装哪个 SDK 取决于编哪个版本**——这是最容易装错的一处
* `for-revit.html` 安装卡同步：前置条件写明 SDK 与 Revit 的对应关系，
  提示改为二选一的 `-p:TargetFrameworks=`

**同时编两个目标需要两套 SDK、两个 Revit 都装**，这是双目标的实际代价，
README 与页面均已注明，只装一个的人按提示编单目标即可。

三轮下来版本这件事的教训（D11 → D14 → D15）：仓库的构建配置与 README 都曾
落后于实际发布状态，我却先后把它们当过真相来源。版本、运行时、兼容性这类
只有作者掌握的事实，应当先问再改。
